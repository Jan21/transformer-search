...
def calculate_metrics(results_dict, tokenizer):
    ...
    colon_id = tokenizer.encode("Command:", add_special_tokens=False)[0]
...
def main(cfg: DictConfig):
    ...
    for i, tok_dataset in tqdm(enumerate(tokenized_datasets)):
        ...
        colon_token_id = tokenizer.encode("Command:", add_special_tokens=False)[0]
        ...
        for b in trange(0, len(prompts_ids), batch_size, desc=f"Generating predictions for {os.path.basename(current_path)}"):
            ...
            for j, output_ids in enumerate(batch_outputs):
                # Find the colon delimiter token in the output
                try:
                    split_index = output_ids.index(colon_token_id)
                    # Find the EOS token
                    end_index = output_ids.index(tokenizer.eos_token_id) if tokenizer.eos_token_id in output_ids else len(output_ids)
                    
                    # Extract everything after the colon up to EOS
