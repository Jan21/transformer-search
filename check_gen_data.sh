#!/bin/bash
#SBATCH --job-name=check_gen_data    # Job name
#SBATCH --output=logs/check_gen_data/pythia_%j.out        # Standard output and error log (%j expands to jobID)
#SBATCH --error=logs/check_gen_data/pythia_%j.err         # Error log
...
singularity exec \
    $SIF \
    python data_generation/test_gen_data.py data/train.json data/test.json

singularity exec \
    $SIF \
    python data_generation/test_gen_data.py data/comma_train.json data/comma_test.json

singularity exec \
    $SIF \
    python data_generation/test_gen_data.py data/concat_train.json data/concat_test.json